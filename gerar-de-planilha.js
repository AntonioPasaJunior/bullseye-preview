...
const BLOG_BASE = process.env.BLOG_BASE || 'https://www.bullseye.ind.br/blog';
const DOMAIN = (process.env.URL || process.env.DOMAIN || 'https://bullseye-blog.netlify.app').replace(/\/+$/, '');
...
<meta property="og:type" content="article"><meta property="og:site_name" content="Bullseye">
...
</head><body style="font-family:Arial,Helvetica,sans-serif;text-align:center;padding:40px;color:#05206D">
<p>Redirecionando para o artigo…</p><p><a href="${redirect}">Clique aqui</a> se não for redirecionado.</p>
</body></html>`;
}

function embed(url, titulo) {
  return `<div id="share-bullseye"></div>
<script>
...
var html = '<div style="font-family:Arial,Helvetica,sans-serif;padding:14px 0;border-top:1px solid #e5e7eb;margin-top:24px">'
  + '<span style="color:#05206D;font-weight:bold;font-size:15px;margin-right:12px">Compartilhe:</span>';
...
html += '</div>'; document.getElementById("share-bullseye").innerHTML = html;
</script>`;
}
